...
function FlipCard({ char, isFlipped, onTap, darkMode }) {
  const hasKundoku = char.kundoku && char.kundokuEx?.length > 0;
    ...
    <div className="cursor-pointer" onClick={onTap} style={{ perspective: '1000px' }}>
      ...
      >
        ...
        <div
          ...
          style={{ backfaceVisibility: 'hidden', height: hasKundoku ? '360px' : '280px' }}
        >
        ...
        {/* 뒷면 */}
        <div
          className={`absolute inset-0 w-full rounded-2xl shadow-lg p-4 pb-6 ${
            darkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'
          }`}
          style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', height: hasKundoku ? '360px' : '280px' }}
        >
          {/* 상단: 한자 + 음뜻 */}
          <div className={`flex items-center justify-center gap-4 mb-3 pb-2 border-b ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
            <span className={`font-serif ${darkMode ? 'text-slate-100' : 'text-slate-800'}`} style={{ fontSize: '48px' }}>
              {char.kanji}
            </span>
            <span className={`text-xl font-bold ${darkMode ? 'text-amber-400' : 'text-slate-600'}`}>
              {char.korean}
            </span>
          </div>

          {/* 음독 */}
          <div className={hasKundoku ? 'mb-3' : ''}>
            ...
            <div className={`pl-3 border-l-2 ${darkMode ? 'border-rose-400/50' : 'border-rose-200'}`}>
              {char.ondokuEx?.map((ex, i) => (
                <div key={i} className="flex items-center gap-3 py-1">
                  <ruby className={`font-medium ${darkMode ? 'text-slate-100' : 'text-slate-800'}`} style={{ fontSize: '32px' }}>
                    {ex.word}
                    <rp>(</rp>
                    <rt className={`font-normal ${darkMode ? 'text-rose-400' : 'text-rose-500'}`} style={{ fontSize: '14px' }}>{ex.reading}</rt>
                    <rp>)</rp>
                  </ruby>
                  <span className={`text-xl ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{ex.meaning}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {hasKundoku && (
            <div>
              ...
              <div className={`pl-3 border-l-2 ${darkMode ? 'border-sky-400/50' : 'border-sky-200'}`}>
                {char.kundokuEx?.map((ex, i) => (
                  <div key={i} className="flex items-center gap-3 py-1">
                    <ruby className={`font-medium ${darkMode ? 'text-slate-100' : 'text-slate-800'}`} style={{ fontSize: '32px' }}>
                      {ex.word}
                      <rp>(</rp>
                      <rt className={`font-normal ${darkMode ? 'text-sky-400' : 'text-sky-500'}`} style={{ fontSize: '14px' }}>{ex.reading}</rt>
                      <rp>)</rp>
                    </ruby>
                    <span className={`text-xl ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{ex.meaning}</span>
                  </div>
...
export default function App() {
  ...
  const goToPrevGroup = () => {
    if (selectedGroup > 0) {
      handleGroupSelect(selectedGroup - 1);
    }
  };

  const goToNextGroup = () => {
    if (selectedGroup < kanjiData.length - 1) {
      handleGroupSelect(selectedGroup + 1);
    }
  };
        ...
        <div className="flex items-center gap-2 mb-4">
          <button
            ...
            className={`p-2 rounded-lg ${
              selectedGroup === 0 ? 'opacity-30' : darkMode ? 'bg-slate-800 text-slate-300' : 'bg-white text-slate-600 border'
            }`}
          >
            ◀
          </button>
          ...
          <button
            ...
            className={`p-2 rounded-lg ${
              selectedGroup === kanjiData.length - 1 ? 'opacity-30' : darkMode ? 'bg-slate-800 text-slate-300' : 'bg-white text-slate-600 border'
            }`}
          >
            ▶
          </button>
