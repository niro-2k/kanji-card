...
const ACCENT_COLOR = '#FF2FFF';       // 마젠타 분홍 (음훈, 선택된 탭)
...
// 단어 단위 루비 컴포넌트 - 한자 위에만 루비 표시
function RubyWord({ word, reading, rubyColor, darkMode, revealed }) {
  if (!revealed) {
    return (
      <span 
  ...
  return (
    ...
    >
      {word}
      <rt style={{ color: rubyColor, fontSize: '12px', fontWeight: 'normal' }}>
        {reading}
      </rt>
    </ruby>
  );
  ...
  return (
    ...
    >
      <div className="flex items-end gap-2">
        ...
        <div 
          className={`text-sm transition-opacity pb-0.5 ${revealed ? 'opacity-100' : 'opacity-0'} ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}
        >
          {ex.meaning}
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto w-full flex flex-col h-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mb-2 shrink-0">
          ...
          <button
            ...
            className="flex-1 py-1.5 px-3 rounded-lg font-medium text-sm text-white"
          ...
          <div className={`mb-2 max-h-40 overflow-y-auto rounded-lg shrink-0 ${darkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
            {kanjiData.map((group, idx) => (
              <button
                ...
                style={selectedGroup === idx ? { backgroundColor: `${ACCENT_COLOR}33`, color: ACCENT_COLOR } : {}}
              >
        ...
        <div className="flex justify-center gap-2 mb-1 shrink-0 flex-wrap">
          {currentGroup.characters.map((char, idx) => (
            <button
              ...
              style={selectedChar === idx ? { backgroundColor: ACCENT_COLOR } : {}}
            >
              {char.kanji}
        ...
        <div className="mt-1 flex justify-center items-center gap-1 shrink-0">
          {currentGroup.characters.map((char, idx) => (
            <div
              ...
              style={idx === selectedChar ? { backgroundColor: ACCENT_COLOR } : {}}
            />
          ))}
        </div>
      </div>
