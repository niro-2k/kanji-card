...
function splitWord(word) {
  ...
  for (const ch of word) {
    const k = isKanji(ch);
    if (currentIsKanji !== null && k !== currentIsKanji) {
...
// 한자 블록에 읽기를 매칭
function matchReadings(word, reading) {
  const segments = splitWord(word);
  const allKanji = segments.every(s => s.isKanji);
  if (allKanji) {
    return [{ text: word, isKanji: true, ruby: reading }];
  }

  const rubyMap = new Array(segments.length).fill(null);
  let tempReading = reading;

  for (let i = segments.length - 1; i >= 0; i--) {
    const seg = segments[i];
    if (!seg.isKanji) {
      if (tempReading.endsWith(seg.text)) {
        tempReading = tempReading.slice(0, -seg.text.length);
        rubyMap[i] = '';
      }
    }
  }
  ...
  for (let i = 0; i < segments.length; i++) {
    const seg = segments[i];
    if (!seg.isKanji && rubyMap[i] === null) {
      if (tempReading.startsWith(seg.text)) {
        tempReading = tempReading.slice(seg.text.length);
        rubyMap[i] = '';
      }
    }
  }

  const kanjiIndices = segments.map((s, i) => s.isKanji && rubyMap[i] === null ? i : -1).filter(i => i >= 0);
  
  if (kanjiIndices.length === 1) {
    rubyMap[kanjiIndices[0]] = tempReading;
  } else if (kanjiIndices.length > 1) {
    let remaining = tempReading;
    for (let j = 0; j < kanjiIndices.length; j++) {
      const idx = kanjiIndices[j];
      if (j === kanjiIndices.length - 1) {
        rubyMap[idx] = remaining;
      } else {
        const nextNonKanjiIdx = segments.findIndex((s, si) => si > idx && !s.isKanji);
        if (nextNonKanjiIdx >= 0) {
          const nextHira = segments[nextNonKanjiIdx].text;
          const splitPos = remaining.indexOf(nextHira);
          if (splitPos >= 0) {
            rubyMap[idx] = remaining.slice(0, splitPos);
            remaining = remaining.slice(splitPos);
          } else {
            rubyMap[idx] = remaining.slice(0, segments[idx].text.length);
            remaining = remaining.slice(segments[idx].text.length);
          }
        } else {
          rubyMap[idx] = remaining.slice(0, segments[idx].text.length);
          remaining = remaining.slice(segments[idx].text.length);
...
function RubyWord({ word, reading, rubyColor, revealed, darkMode }) {
  ...
  const parts = matchReadings(word, reading);
    ...
    >
      {parts.map((p, i) =>
        p.isKanji && p.ruby ? (
          <ruby key={i}>
            {p.text}
            <rt style={{ color: rubyColor, fontSize: '12px', fontWeight: 'normal' }}>{p.ruby}</rt>
          </ruby>
        ) : (
